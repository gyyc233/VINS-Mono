...
class ImageFrame
{
    public:
        ...
        // {{特征点id,[<相机索引,特征点信息(x,y,z,u,v,velocity_x, velocity_y)>,<>,<>]},{},{}}
        // 该结构便于在多相机、多帧之间进行特征匹配与三角化
        std::map<int, std::vector<std::pair<int, Eigen::Matrix<double, 7, 1>> > > points; // 图像特征点数据。每个 feature id 对应多个观测
        double t; // 当前帧时间戳
...
/// @note 包含以下任务: 1. 陀螺仪偏置校正（Gyro Bias Correction）2. 重力向量优化（Gravity Optimization）3. 尺度恢复（Scale Recovery）4. 绝对尺度对齐（Aligning to World Frame）
