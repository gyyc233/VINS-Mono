...
/// @brief 通过视觉与 IMU 数据联合估计陀螺仪偏置（gyroscope bias）的初始值，并更新所有帧的预积分数据
/// @note 利用视觉估计的帧间旋转（q_ij）与 IMU 预积分中的预测旋转之间的差异，通过最小二乘，反推出最可能的陀螺仪偏置
/// @param all_image_frame 图像帧集合
/// @param Bgs 每帧对应的陀螺仪偏置
void solveGyroscopeBias(map<double, ImageFrame> &all_image_frame, Vector3d* Bgs)
{
    // 在 VIO 中，IMU 的陀螺仪测量中通常包含一个缓慢变化的偏置（bias）。这个偏置如果不准确，会导致预积分误差累积，从而影响位姿估计
    Eigen::Matrix3d A;
    Eigen::Vector3d b;
    ...
    for (frame_i = all_image_frame.begin(); next(frame_i) != all_image_frame.end(); frame_i++)
    {
        ...
        // 计算 IMU 预测的旋转与视觉估计的旋转之间的误差（四元数），提取虚部作为误差向量
...
/// @brief 构造一个与重力向量g_0正交的二维切空间基底（tangent basis），即返回两个单位正交向量，它们构成重力方向的切平面上的一组基
/// @param g0 当前估计的重力向量
/// @return 
Eigen::MatrixXd TangentBasis(Eigen::Vector3d &g0)
{
    ...
    // 希望找到两个单位向量 b 和 c，他们都与归一化后的 g0 垂直，且b c 相互正交，构成右手系
    Eigen::Vector3d b, c;
...
/// @brief 在固定尺度的前提下，通过非线性优化（高斯牛顿）逐步 refine（精细化）重力方向的估计值
...
void RefineGravity(std::map<double, ImageFrame> &all_image_frame, Eigen::Vector3d &g, Eigen::VectorXd &x)
{
    ...
    // 迭代四次
    // 若数据质量较差（如 IMU 噪声大、视觉跟踪不稳定），重力方向误差大，初始估计不好，则需要增加迭代次数
    for(int k = 0; k < 4; k++)
    {
...
/// @note gaussian-newton 构建残差项,使用LDLT求解Ax=b，优化状态变量（包括重力向量与尺度因子）
/// @param all_image_frame 图像帧集合
/// @param g output 重力向量
