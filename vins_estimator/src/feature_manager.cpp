...
void FeatureManager::removeBackShiftDepth(Eigen::Matrix3d marg_R, Eigen::Vector3d marg_P, Eigen::Matrix3d new_R, Eigen::Vector3d new_P)
{
         ...
         it != feature.end(); it = it_next)
    {
        ...
        {
            // 该特征点首次出现的帧号在最老帧
            Eigen::Vector3d uv_i = it->feature_per_frame[0].point; // 保存最早帧的归一化图像坐标 uv_i
            it->feature_per_frame.erase(it->feature_per_frame.begin()); // 删除最早帧对该特征点的观测
            ...
            {   // 深度一致性维护
                Eigen::Vector3d pts_i = uv_i * it->estimated_depth; // 重建该点在旧参考帧下的3D坐标
                Eigen::Vector3d w_pts_i = marg_R * pts_i + marg_P; // 转到世界坐标系
                Eigen::Vector3d pts_j = new_R.transpose() * (w_pts_i - new_P); // 转到新参考帧下
