...
/// @brief IMU预积分类 ：加速度计、陀螺仪、线性加速度计ba、陀螺仪bg、雅克比矩阵初始化、协方差矩阵15*15、dt、PVQ
class IntegrationBase
{
  public:
    ...
    /// @param _linearized_ba 初始时刻加速计的零偏
    /// @param _linearized_bg 初始时刻角速度计的零偏
    ...
    /// @brief 优化过程中Bias会更新，需要根据新的bias重新计算预积分
    ...
    */
    ...
    /// @brief IMU预积分传播方程
    /// @details 积分计算两个关键帧之间IMU测量的变化量：旋转delta_q 速度delta_v 位移delta_p, 加速度的bias linearized_ba 陀螺仪的Bias linearized_bg, 同时维护更新预积分的Jacobian和Covariance,计算优化时必要的参数
    /// @param _dt 
    /// @param _acc_1 
    /// @param _gyr_1 
    ...
    /// @details Pi，Qi，Vi，Bai，Bgi  [前一次预积分结果]; Pj，Qj，Vj，Baj，Bgj  [后一次预积分结果]
