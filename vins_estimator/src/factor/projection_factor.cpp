...
bool ProjectionFactor::Evaluate(double const *const *parameters, double *residuals, double **jacobians) const
{
    ...
    Eigen::Vector3d pts_imu_j = Qj.inverse() * (pts_w - Pj); // 世界坐标系转到j帧坐标系下
    Eigen::Vector3d pts_camera_j = qic.inverse() * (pts_imu_j - tic); // 转到j帧时的相机坐标系下
...
#else
    // pts_i 和 pts_j 已经是归一化相机坐标，相机内参已在前端去畸变和坐标转换时应用过
    ...
    if (jacobians)
    {
        ...
        if (jacobians[0])
        {
            // 如果用户请求计算雅可比矩阵（即 jacobians != nullptr 且 jacobians[0] != nullptr），则进入计算流程
            Eigen::Map<Eigen::Matrix<double, 2, 7, Eigen::RowMajor>> jacobian_pose_i(jacobians[0]);

            // 由于四元数是4维，所以后6维用于构建局部更新空间（由 LocalParameterization 处理），最后一维为常量或忽略
