...
/// @brief 将一个残差块信息（ResidualBlockInfo）添加到当前的边缘化结构中，用于后续进行 Schur 补和变量边缘化
/// @param residual_block_info 
void MarginalizationInfo::addResidualBlockInfo(ResidualBlockInfo *residual_block_info)
{
    factors.emplace_back(residual_block_info); // factors 保存了所有参与边缘化的残差项
...
/// @brief 边缘化流程的预处理阶段，负责对所有残差块进行线性化，并保存当前参数值，为后续边缘化提供基础数据支撑
void MarginalizationInfo::preMarginalize()
{
    // 通过调用 addResidualBlockInfo() 已经确定优化变量的数量、存储位置、长度以及待优化变量的数量以及存储位置
    // preMarginalize() 进行预处理， 
    // 得到每次 IMU 和视觉观测(cost_function)对应的参数块(parameter_blocks)，雅可比矩阵(jacobians)，残差值(residuals)

    for (auto it : factors)
    {
        it->Evaluate(); // 利用多态性调用各个损失函数中的重载函数Evaluate()
        ...
        for (int i = 0; i < static_cast<int>(block_sizes.size()); i++)
        {
            ...
            // parameter_block_data是整个优化变量的数据
            if (parameter_block_data.find(addr) == parameter_block_data.end())
            {
                // 通过之前优化变量的数据的地址和新开辟的内存数据进行关联
...
/// @note 在多个线程中并行计算增量方程中的 Hessian 矩阵 $ A = J^T J $ 和梯度 $ b = J^T r $，供后续 Schur 补使用，并行机制: 多线程分割残差块，加速构建过程
/// @param threadsstruct 
/// @return 
void* ThreadsConstructA(void* threadsstruct)
{
    // p 是一个结构体指针，包含当前线程需要处理的残差块、Hessian 矩阵 A 和梯度向量 b
...
/// @brief 开启多线程构建残差增量矩阵H和b ，使用schur消元，从H,b中恢复出线性化雅克比和残差
void MarginalizationInfo::marginalize()
{
    ...
    Eigen::MatrixXd A(pos, pos); // s整个矩阵大小 --- 没有边缘化之前的矩阵
...
std::vector<double *> MarginalizationInfo::getParameterBlocks(std::unordered_map<long, double *> &addr_shift)
{
    ...
    for (const auto &it : parameter_block_idx)
    {
        ...
        if (it.second >= m)
        {
            ...
            keep_block_idx.push_back(parameter_block_idx[it.first]); // 参数块在整个变量空间中的偏移量
            keep_block_data.push_back(parameter_block_data[it.first]); // 保存的原始参数值（来自preMarginalize）
            keep_block_addr.push_back(addr_shift[it.first]); // 根据输入映射 addr_shift 获取新地址
        }
    }
...
MarginalizationFactor::MarginalizationFactor(MarginalizationInfo* _marginalization_info):marginalization_info(_marginalization_info)
{
    ...
    // 构造完成后，MarginalizationFactor 对象就可以作为普通因子加入到 Ceres 优化问题
    // problem.AddResidualBlock(new MarginalizationFacto(marginalization_info), NULL, keep_block_addr);
};

// 该函数根据当前参数与边缘化信息构造残差，并提供对应的雅可比矩阵，供 Ceres 进行非线性优化
bool MarginalizationFactor::Evaluate(double const *const *parameters, double *residuals, double **jacobians) const
{
    ...
    for (int i = 0; i < static_cast<int>(marginalization_info->keep_block_size.size()); i++)
    {
        ...
        Eigen::VectorXd x = Eigen::Map<const Eigen::VectorXd>(parameters[i], size); // 当前参数值

        // x0表示marg时参数块变量的值(即xb)
        Eigen::VectorXd x0 = Eigen::Map<const Eigen::VectorXd>(marginalization_info->keep_block_data[i], size); // 线性化点的参数值，来自 preMarginalize）
