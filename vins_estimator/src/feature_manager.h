...
class FeatureManager
{
  public:
    ...
    /// @note 1. 添加新的特征点到特征管理器; 2. 维护每个特征点在多帧中的观测信息; 3. 计算并判断视差是否足够大，以决定当前帧是否适合进行初始化（如三角化、位姿估计等）
    ...
    ///@param x 根据输入的向量 x 更新特征点的深度值。这个输入向量 x 通常来自于后端优化器的结果，其中包含了每个有效特征点的逆深度值（inverse depth）
    ...
    /// @note 滑动窗口机制限制了历史帧数量, 边缘化会把最老帧的状态从优化问题中移除, 但某些特征点可能还存在于后续帧中，不能直接丢弃, 将这些点更新到新参考帧下，避免重复三角化
