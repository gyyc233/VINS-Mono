...
class Estimator
{
  public:
    ...
    /// @note 设置重定位帧的信息，以便在系统发生故障或需要回环检测时，能够基于已知地图信息进行相对位姿估计和漂移校正
    ...
    /// @brief 在滑动窗口中寻找一个参考帧（记为 i），该帧与最新帧（WINDOW_SIZE）之间具有足够的特征点对应关系和视差（parallax）
    /// @param relative_R 
    /// @param relative_T 
    /// @param l 参考帧索引
    /// @return 
    ...
    // 优化参数块
    // 滑动窗口排列，假设 WINDOW_SIZE = 10
    // para_Pose[0] 最老帧，最早进入窗口的帧
    // para_Pose[9]	次新帧（倒数第二新的帧）
    // para_Pose[10]	最新帧（最新进入的帧）
    double para_Pose[WINDOW_SIZE + 1][SIZE_POSE]; // [x,y,z,qx,qy,qz,qw]
    double para_SpeedBias[WINDOW_SIZE + 1][SIZE_SPEEDBIAS]; // [v_x,v_y,v_z,b_a_x,b_a_y,b_a_z,b_g_x,b_g_y,b_g_z]
    double para_Feature[NUM_OF_F][SIZE_FEATURE]; // 特征点逆深度或真实深度
    double para_Ex_Pose[NUM_OF_CAM][SIZE_POSE]; // camera-imu extrinsic translation [tic, ric]
    ...
    // ImageFrame 包含 a. 特征点信息；b. 对应imu预积分器；c. 是否是关键帧；d. 当前帧相对参考帧的RT; e. 图像时间戳等
